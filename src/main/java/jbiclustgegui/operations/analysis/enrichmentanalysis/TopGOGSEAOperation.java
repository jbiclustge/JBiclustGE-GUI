 ...
 */
@Operation(name="Run GSEA With TopGO",description="Execute TopGO",enabled = false)
public class TopGOGSEAOperation {
	...
	/** The analyser. */
	private TopGOEnrichmentAnalyser analyser;
	...
	public void setTopGOSettings(Properties props) {
		this.props=props;
	}
	...
	private void run() throws Exception {
		...
		analyser= new TopGOEnrichmentAnalyser(results, props);
		analyser.addAnalyserPropertyChangeListener(progressview);
		analyser.run();

		GSEAConfigurationContainer configcont=new GSEAConfigurationContainer(props,GSEAAnalyserType.TopGO);
	...
	public void cancel(){
		...
		if(result==JOptionPane.YES_OPTION) {
			stopped=true;
			if(analyser!=null) {
				progressview.setCurrentTask("Stopping TopGO...");
				progressview.setCurrentSubTask("Stopping TopGO...");
