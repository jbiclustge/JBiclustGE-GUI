...
/**
 * The Class OntologizerGSEAOperation.
 */
@Operation(name="Run GSEA With Ontologizer",description="Execute Ontologizer",enabled = false)
public class OntologizerGSEAOperation {
	...
	/** The analyser. */
	private OntologizerEnrichmentAnalyser analyser;
	...
	/**
	 * Sets the ontologizer settings.
	 *
	 * @param props the new ontologizer settings
	 */
	@Port(name="Settings analyser",direction=Direction.INPUT,order=3)
	public void setOntologizerSettings(Properties props) {
		this.props=props;
	}
	...
	private void run() throws Exception {
		...
		analyser= new OntologizerEnrichmentAnalyser(results, props);
		analyser.addAnalyserPropertyChangeListener(progressview);
		analyser.run();

		GSEAConfigurationContainer configcont=new GSEAConfigurationContainer(props,GSEAAnalyserType.Ontologizer);
	...
	public void cancel(){
		...
		if(result==JOptionPane.YES_OPTION) {
			stopped=true;
			if(analyser!=null) {
				progressview.setCurrentTask("Stopping Ontologizer...");
