...
/**
 * The Class RunISAOperation.
 */
@Operation(name="Run ISA",description="Execute ISA Algorithm",enabled = false)
public class RunISAOperation extends AbstractMethodOperation{
	

	/** The method. */
	private RIsaMethod method;
	...
	protected BiclusterList runMethod() {
		method=new RIsaMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(ISAResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		//String dataid="ISA_result_"+nelems+" "+MTUTimeUtils.getCurrentDateAndTime("yyyy-MM-dd_HH.mm.ss");
		
		String dataid="ISA_result_"+nelems;
		
		ISAResultsBox res=new ISAResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,ISAResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
