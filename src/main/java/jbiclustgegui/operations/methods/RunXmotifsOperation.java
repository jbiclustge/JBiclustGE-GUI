...
/**
 * The Class RunXmotifsOperation.
 */
@Operation(name="Run xMOTIFs",description="Execute xMOTIFs Algorithm",enabled = false)
public class RunXmotifsOperation extends AbstractMethodOperation{
	
    /** The method. */
    private RXMOTIFSMethod method;
	...
	protected BiclusterList runMethod() {
		method=new RXMOTIFSMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(XmotifsResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="xMOTIFs_result_"+nelems;
		
		XmotifsResultsBox res=new XmotifsResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,XmotifsResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
