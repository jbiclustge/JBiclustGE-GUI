...
/**
 * The Class RunCoalesceOperation.
 */
@Operation(name="Run Coalesce",description="Execute COALESCE Algorithm",enabled = false)
public class RunCoalesceOperation extends AbstractMethodOperation{
	
	/** The method. */
	private COALESCEMethod method;
	...
	protected BiclusterList runMethod() {
		method=new COALESCEMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(COALESCEResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="COALESCE_result_"+nelems;
		
		COALESCEResultsBox res=new COALESCEResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,COALESCEResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
