...
/**
 * The Class RunFabiaPOperation.
 */
@Operation(name="Run FabiaP",description="Execute FabiaP Algorithm",enabled = false)
public class RunFabiaPOperation extends AbstractMethodOperation{
	
	/** The method. */
	private RFabiaPMethod method;
	...
	protected BiclusterList runMethod() {
		method=new RFabiaPMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(FabiaPResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="FabiaP_result_"+nelems;
		
		FabiaPResultsBox res=new FabiaPResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,FabiaPResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
