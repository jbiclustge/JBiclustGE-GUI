...
/**
 * The Class RunQubicOperation.
 */
@Operation(name="Run QuBic",description="Execute QuBic Algorithm",enabled = false)
public class RunQubicOperation extends AbstractMethodOperation{
	
	/** The method. */
	private QuBicMethod method;
	...
	protected BiclusterList runMethod() {
		method=new QuBicMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(QubicResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="QuBic_result_"+nelems;
		
		QubicResultsBox res=new QubicResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,QubicResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
