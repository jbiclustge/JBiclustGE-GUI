...
/**
 * The Class RunBiclicOperation.
 */
@Operation(name="Run BICLIC",description="Execute BICLIC Algorithm",enabled = false)
public class RunBiclicOperation extends AbstractMethodOperation{
	
	/** The method. */
	private RBiclicMethod method;
	...
	protected BiclusterList runMethod() {
		method=new RBiclicMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(BiclicResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="BICLIC_result_"+nelems;
		
		BiclicResultsBox res=new BiclicResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj, BiclicResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
