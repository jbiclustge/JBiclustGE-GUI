...
/**
 * The Class RunCPBOperation.
 */
@Operation(name="Run CPB",description="Execute CPB Algorithm",enabled = false)
public class RunCPBOperation extends AbstractMethodOperation{
	
	/** The method. */
	private CPBMethod method;
	...
	protected BiclusterList runMethod() {
		method=new CPBMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(CPBResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="CPB_result_"+nelems;
		
		CPBResultsBox res=new CPBResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,CPBResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
