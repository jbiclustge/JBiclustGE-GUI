...
/**
 * The Class RunFabiaOperation.
 */
@Operation(name="Run Fabia",description="Execute Fabia Algorithm",enabled = false)
public class RunFabiaOperation extends AbstractMethodOperation{
	
    /** The method. */
    private RFabiaMethod method;
	...
	protected BiclusterList runMethod() {
		method=new RFabiaMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(FabiaResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="Fabia_result_"+nelems;
		
		FabiaResultsBox res=new FabiaResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,FabiaResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
