...
/**
 * The Class RunCCOperation.
 */
@Operation(name="Run Cheng&Church",description="Execute Cheng&Church Algorithm",enabled = false)
public class RunCCOperation extends AbstractMethodOperation{
	
    /** The method. */
    private RBCCCMethod method;
	...
	protected BiclusterList runMethod() {
		method=new RBCCCMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(ChengAndChurchResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="Cheng&Church_result_"+nelems;
		
		ChengAndChurchResultsBox res=new ChengAndChurchResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,ChengAndChurchResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
