...
/**
 * The Class RunBicFinderOperation.
 */
@Operation(name="Run BicFinder",description="Execute BicFinder Algorithm",enabled = false)
public class RunBicFinderOperation extends AbstractMethodOperation{
	

	/** The method. */
	private BicFinderMethod method;
	...
	protected BiclusterList runMethod() {
		method=new BicFinderMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(BicFinderResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="BicFinder_result_"+nelems;
		
		BicFinderResultsBox res=new BicFinderResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj, BicFinderResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
