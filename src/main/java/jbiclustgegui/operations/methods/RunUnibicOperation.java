...
/**
 * The Class RunUnibicOperation.
 */
@Operation(name="Run UniBic",description="Execute UniBic Algorithm",enabled = false)
public class RunUnibicOperation extends AbstractMethodOperation{

	/** The method. */
	private UnibicMethod method;
	...
	protected BiclusterList runMethod() {
		...
		method=new UnibicMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(UnibicResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="UniBic_result_"+nelems;
		
		UnibicResultsBox res=new UnibicResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,UnibicResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
