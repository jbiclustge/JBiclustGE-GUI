...
/**
 * The Class RunDebiOperation.
 */
@Operation(name="Run DeBi",description="Execute DeBi Algorithm",enabled = false)
public class RunDebiOperation extends AbstractMethodOperation{
	
	/** The method. */
	private DebiMethod method;
	...
	protected BiclusterList runMethod() {
		method=new DebiMethod(expressionset);
	...
	protected String addResultsToClipboard(BiclusterList results) throws InvalidElementListException {
		...
		IElementList<IBiclusteringMethodResult> elems=proj.getBiclusteringMethodResultElementListByClass(DebiResultsBox.class);
		int nelems=0;
		if(elems!=null)
			nelems=elems.size();
		
		nelems=nelems+1;
		String dataid="DeBi_result_"+nelems;
		
		DebiResultsBox res=new DebiResultsBox(dataid, proj, results);
		res.setDate(LocalDateTime.now());
		GenericOperation.addBiclusteringResult(proj,DebiResultsBox.class, res, res.getMethod().getAlgorithmName()+" Results");
	    return dataid;
	}
